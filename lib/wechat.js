...
exports = module.exports = class wechat {
  ...
  login() {
    return this.axios({
      ...
      this.skey = res.data.match(/<skey>(.*)<\/skey>/)
      this.sid = res.data.match(/<wxsid>(.*)<\/wxsid>/)
      this.uin = res.data.match(/<wxuin>(.*)<\/wxuin>/)
      this.passTicket = res.data.match(/<pass_ticket>(.*)<\/pass_ticket>/)
