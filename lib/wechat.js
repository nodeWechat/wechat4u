...
class WechatEmitter extends EventEmitter {}
...
exports = module.exports = class wechat {
  constructor() {
    ...
    this.wechatEmitter = new WechatEmitter()
    this.wechatEmitter.on('logout', ()=>{})
  ...
  syncPolling() {
    this.syncCheck().then(state => {
      if (state.retcode == '1100' || state.retcode == '1101') {
        ...
        this.wechatEmitter.emit('logout')
  ...
  on(event, callback) {
    if(typeof event == 'string' && typeof callback == "function") {
      this.wechatEmitter.on(event, callback)
    } else {
